...
class SslPinningInterceptor extends Interceptor {
  ...
  Future<void> onRequest(
    ...
    try {
      ...
      if (isValid) {
        AppLogger.debug(
          'SslPinningInterceptor: Certificate validated for $host',
        );
        return handler.next(options);
      } else {
      ...
      final socket = await SecureSocket.connect(
        ...
        onBadCertificate: (X509Certificate cert) {
          ...
          final isAllowed = allowedSHAFingerprints.any(
            (allowed) =>
                _normalizeFingerprint(allowed) == normalizedFingerprint,
          );
          ...
          if (!isAllowed) {
            ...
            AppLogger.debug('SslPinningInterceptor: Received: $fingerprint');
            AppLogger.debug(
  ...
  Uint8List hash(Uint8List message) {
    ...
    for (var i = 0; i < paddedMessage.length; i += 64) {
      ...
      for (var t = 0; t < 64; t++) {
        ...
        final ch = (e & f) ^ ((~e) & g);
        final temp1 = _add32(
          _add32(_add32(_add32(h, sigma1), ch), _k[t]),
          w[t],
        );
