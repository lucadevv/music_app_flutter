...
class LibraryRepositoryImpl implements LibraryRepository {
  ...
  Future<Either<AppException, LibrarySummaryEntity>> getSummary() async {
    try {
      ...
      return Right(LibrarySummaryEntity(
  ...
  @override
  Future<Either<AppException, List<Song>>> getFavoriteSongs({int page = 1, int limit = 20}) async {
    try {
      final data = await _remoteDataSource.getFavoriteSongs(page: page, limit: limit);
  ...
  @override
  Future<Either<AppException, List<FavoritePlaylistEntity>>> getFavoritePlaylists({
    int page = 1,
    int limit = 20,
  }) async {
    try {
      final data = await _remoteDataSource.getFavoritePlaylists(page: page, limit: limit);
      final response = FavoritePlaylistsResponse.fromJson(data);
      final playlists = response.playlists.map((p) => FavoritePlaylistEntity(
        id: p.playlistId,
        name: p.name,
  ...
  Future<Either<AppException, List<FavoriteGenreEntity>>> getFavoriteGenres({
    ...
    try {
      final data = await _remoteDataSource.getFavoriteGenres(page: page, limit: limit);
      final response = FavoriteGenresResponse.fromJson(data);
      final genres = response.genres.map((g) => FavoriteGenreEntity(
        id: g.genreId,
        name: g.name,
        params: g.params,
      )).toList();
