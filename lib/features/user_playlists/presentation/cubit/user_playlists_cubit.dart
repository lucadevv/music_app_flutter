...
class UserPlaylistsCubit extends Cubit<UserPlaylistsState> with BaseBlocMixin {
  ...
  Future<void> loadAllPlaylists() async {
    ...
    try {
      ...
      final playlists = <PlaylistItem>[
        ...userPlaylists.data.map((p) => PlaylistItem(
          id: p.id,
          name: p.name,
        ...
        ...uniqueFavorites.map((p) => PlaylistItem(
          id: p.id,
          name: p.name,
          thumbnail: p.thumbnail,
  ...
  Future<void> createPlaylist(String name) async {
    try {
      ...
      final errorMessage = e.toString();
      emit(state.copyWith(status: UserPlaylistsStatus.failure, errorMessage: errorMessage));
    }
  }
}
