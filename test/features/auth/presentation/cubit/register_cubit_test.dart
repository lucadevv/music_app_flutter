...
void main() {
  ...
  group('RegisterCubit', () {
    ...
    blocTest<RegisterCubit, RegisterState>(
      ...
      build: () {
        when(() => mockRepository.register(any(that: isA<RegisterRequest>())))
      ...
      expect: () => [
        isA<RegisterState>()
            .having((s) => s.status, 'status', RegisterStatus.loading),
        isA<RegisterState>()
      ...
      verify: (_) {
        verify(() => mockRepository.register(any(that: isA<RegisterRequest>())))
            .called(1);
      },
    );
    ...
    blocTest<RegisterCubit, RegisterState>(
      ...
      build: () {
        when(() => mockRepository.register(any(that: isA<RegisterRequest>())))
            .thenAnswer((_) async => Left(createTestValidationException('Email already exists')));
        return registerCubit;
      },
      act: (cubit) => cubit.register(createTestRegisterRequest()),
      expect: () => [
        ...
        isA<RegisterState>()
            .having((s) => s.status, 'status', RegisterStatus.failure)
            .having((s) => s.errorMessage, 'errorMessage', 'Email already exists'),
      ],
    );
    ...
    blocTest<RegisterCubit, RegisterState>(
      ...
      expect: () => [
        isA<RegisterState>().having((s) => s.status, 'status', RegisterStatus.loading),
        isA<RegisterState>().having((s) => s.status, 'status', RegisterStatus.success),
      ],
    );
